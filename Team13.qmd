---
title: "outlier제거(IQR1.5기준) + scaling 및 지역별, 연도별 평단가 분산분석 + 다중회귀분석(종속변수 SalePrice)"
...
```{r}
dataH = dataG
dataH$Days = scale(dataH$Days)
dataH$Interior = scale(dataH$Interior)
dataH$Bed = scale(dataH$Bed)
dataH$Bath = scale(dataH$Bath)
dataH$Rooms = scale(dataH$Rooms)
dataH$Condo = scale(dataH$Condo)
dataH$Tax = scale(dataH$Tax)
dataH$HousePriceIndex = scale(dataH$HousePriceIndex)
```
...
LM2 = lm(SalePrice ~ Interior + Zone + HousePriceIndex + Days + Rooms + Bed + Bath + Condo + Tax + RC, dataH)
```
...
```{r}
predict(LM3, newdata = data.frame(Interior = 1040, Zone = "2Zone",HousePriceIndex = 99.38, YearM = "1994", Days = 24, Rooms = 5, Bed = 2, Bath = 1, Condo = 175 , Tax = 1121 , RC = "1"), interval = "prediction")
```
```{r}
200726.4 - 136353.8
```